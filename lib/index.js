...
function repositoryUrl(candidate, suffix) {
	...
	if (owner === void 0 || repository === void 0) throw new Error("remote candidate source is not a GitHub repository");
	return `https://api.github.com/repos/${encodeURIComponent(owner)}/${encodeURIComponent(repository)}${suffix}`;
}
function rawUrl(candidate, path) {
...
function isTreeItem(value) {
	...
	const item = value;
	if (typeof item.path !== "string" || item.path.length === 0 || item.path.length > MAX_REPOSITORY_PATH_LENGTH || item.path.startsWith("/") || item.path.includes("\\") || item.path.includes("\0") || item.path.split("/").some((segment) => segment.length === 0 || segment === "." || segment === "..")) return false;
	return item.type === "blob" || item.type === "tree" || item.type === "commit";
}
async function fetchSkill(candidate, path, signal) {
	...
	if (Number.isFinite(contentLength) && contentLength > MAX_REMOTE_SKILL_BYTES$1) throw new Error(`remote SKILL.md exceeds ${MAX_REMOTE_SKILL_BYTES$1} bytes during uniqueness check`);
	const raw = await response.text();
	if (Buffer$1.byteLength(raw, "utf8") > MAX_REMOTE_SKILL_BYTES$1) throw new Error(`remote SKILL.md exceeds ${MAX_REMOTE_SKILL_BYTES$1} bytes during uniqueness check`);
	try {
		return {
			name: parseSkillMarkdown(raw, `/skillflux-remote-uniqueness/${path}`).name,
			path,
...
async function verifyUniqueRemoteSkill(candidate, signal) {
	...
	const paths = payload.tree.filter((item) => item.type === "blob").map((item) => item.path).filter((path) => path === "SKILL.md" || path.endsWith("/SKILL.md"));
	if (paths.length > MAX_REPOSITORY_SKILL_FILES) throw new Error(`cannot prove remote Skill uniqueness across more than ${MAX_REPOSITORY_SKILL_FILES} SKILL.md files`);
	const indexed = [];
	for (let offset = 0; offset < paths.length; offset += FETCH_CONCURRENCY) {
		const batch = await Promise.all(paths.slice(offset, offset + FETCH_CONCURRENCY).map(async (path) => await fetchSkill(candidate, path, signal)));
		signal?.throwIfAborted();
	...
	async install(candidate, signal) {
		...
		try {
			...
			const execute = async (env) => {
				if (this.options.runInstaller !== void 0) await withAbort(this.options.runInstaller({
...
var SkillFluxService = class extends Service {
	...
	async executeCommand(invocation) {
		...
		if (parts.length === 1 && parts[0] === "status") {
			...
			const discovery = `Remote discovery: ${this.config.remoteDiscovery}; providers ${this.config.remoteProviders.map((provider) => provider === "github" && !this.remote.githubSearchEnabled ? "github (token unavailable)" : provider).join(", ")}; evidence policy ${this.config.remoteTrustPolicy}; quality >= ${this.config.remoteMinQualityScore}; stars >= ${this.config.remoteMinStars}; recent window ${this.config.remoteRecentActivityDays} days; ${this.config.remoteBlockedOwners.length} blocked owner(s).`;
		...
		if (this.config.approvalPolicy === "automatic") {
			...
			for (const candidate of remote.slice(0, this.config.remoteAutoMountLimit)) {
				...
				try {
					...
					this.assertMountCurrent(state, generation, candidate.name, mountEpoch);
					if (error instanceof ExpiredAgentStateError) throw error;
					const outcome = error instanceof CatalogBudgetExceededError ? "budget-skipped" : deadline.aborted || error instanceof Error && error.name === "TimeoutError" ? "mount-timeout" : "mount-failed";
					state.lastRouting.push({
						...routingTrace(candidate, turn),
						outcome
					});
					state.candidates.delete(candidate.id);
					this.runtimeCtx.logger.warn(`SkillFlux automatic remote mount ${outcome} for ${candidate.name} from ${candidate.source}: ${errorMessage(error)}`);
					if (deadline.aborted) break;
				}
			}
		}
	...
	async mountCandidate(state, candidate, signal, expectedGeneration = state.generation, expectedMountEpoch = state.mountEpochs.get(candidate.name) ?? 0) {
		...
		try {
			...
			this.assertCatalogBudget(state, definition);
			this.assertMountCurrent(state, expectedGeneration, candidate.name, expectedMountEpoch);
			const releaseActiveLease = await this.cache.createActiveLease(entry.manifest.cacheId);
			try {
				cacheSignal?.throwIfAborted();
				this.assertMountCurrent(state, expectedGeneration, candidate.name, expectedMountEpoch);
			} catch (error) {
				await this.trackActiveLeaseCleanup(releaseActiveLease);
				throw error;
			}
			let dispose;
			try {
			...
			const disposeMounted = () => {
				...
				try {
					...
					activeCleanupTask = this.trackActiveLeaseCleanup(releaseActiveLease);
				}
			};
			...
			try {
				...
				if (state.active.get(mounted.name) === mounted) {
					...
					try {
						...
						this.runtimeCtx.logger.warn(`SkillFlux cancelled mount rollback failed: ${errorMessage(disposeError)}`);
					}
			...
			if (candidate.origin === "remote") {
				this.cachePruneSessions.add(state.agent.session);
	...
	disposeAgent(agent) {
		...
		if (session !== void 0 && ![...this.states].some((item) => item.agent.session === session)) this.scheduleSessionCachePrune(session);
	}
