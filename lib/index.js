...
function remoteQualityEvidence(input) {
	...
	const tierTrust = input.registryTier === "official" ? 3 : input.registryTier === "verified" ? 2 : input.registryTier === "community" ? 1 : 0;
	const trust = (input.trustedSource ? 10 : 0) + (input.organizationOwned ? 3 : 0) + (input.hasLicense ? 2 : 0) + tierTrust;
...
function currentCandidateTrust(candidate, config) {
	if (candidate.origin === "mcp") return config.mcpTrustedServers.includes(candidate.serverLabel.toLocaleLowerCase("en-US")) ? "trusted" : "community";
...
function candidateGovernanceReason(candidate, config) {
	...
	if (candidate.origin === "mcp") {
		...
		if (config.mcpBlockedServers.includes(label)) return `MCP server "${candidate.serverLabel}" is blocked by mcpBlockedServers`;
		const trustLevel = currentCandidateTrust(candidate, config);
		if (!remoteTrustPolicyAllows(trustLevel, config.remoteTrustPolicy)) return `MCP candidate evidence level "${trustLevel}" does not satisfy remoteTrustPolicy "${config.remoteTrustPolicy}"`;
		return;
	}
	const owner = repositoryOwner(candidate.source);
...
function registerApprovalGate(ctx, host) {
	ctx.on("tools/pre-execute", async (exec, next) => {
		if (exec.name === "skillflux_mount") {
			...
			if (candidate.origin !== "remote" && candidate.origin !== "mcp" || host.config.approvalPolicy === "automatic") return downstream;
			...
			return {
				kind: "ask",
				reason: candidate.origin === "remote" ? `Install remote skill ${candidate.skillId} from ${candidate.source} at immutable commit ${candidate.ref}?` : `Load MCP skill ${candidate.name} from server ${candidate.source} at content-bound digests ${candidate.contentBoundKey.slice(0, 12)}?`
			};
		}
...
function cacheCandidates(entries) {
	return entries.map(({ manifest }) => {
		if (manifest.origin === "mcp" && manifest.mcp !== void 0) return {
...
async function activateCandidate(host, state, candidate, signal, expectedGeneration = state.generation, expectedMountEpoch = state.mountEpochs.get(candidate.name) ?? 0) {
	...
	try {
		...
		if (candidate.origin === "cache") {
			...
			entry = await host.cache.installMcp(candidate, (uri, readSignal) => host.mcpReader(candidate.serverLabel, uri, readSignal), cacheSignal);
			cacheSignal?.throwIfAborted();
			host.assertMountCurrent(state, expectedGeneration, candidate.name, expectedMountEpoch);
		} else {
...
function mergeEvidence(previous, item) {
	...
	return {
		...
		...item.totalLoadedBodyTokens === void 0 && previous.totalLoadedBodyTokens === void 0 ? {} : { totalLoadedBodyTokens: safeSum(previous.totalLoadedBodyTokens ?? 0, item.totalLoadedBodyTokens ?? 0) }
	};
}
...
function evictionOrder$1(left, right) {
	...
	if (left.totalLoadedBodyTokens !== right.totalLoadedBodyTokens) return right.totalLoadedBodyTokens - left.totalLoadedBodyTokens;
...
//#endregion
//#region src/mcp-source.ts
const MCP_SKILLS_EXTENSION = "io.modelcontextprotocol/skills";
const MCP_MAX_RESOURCES_PER_SKILL = 512;
const MCP_MAX_SKILL_BYTES = 16777216;
const MCP_MAX_LIST_PAGES = 10;
...
function assertMcpServerLabel(label) {
	if (!MCP_SERVER_LABEL.test(label)) throw new Error(`invalid host-assigned MCP server label "${label}"`);
}
function isRecord(value) {
	return typeof value === "object" && value !== null && !Array.isArray(value);
}
function isSafeUri(value) {
	return typeof value === "string" && value.length > 0 && value.length <= 4096 && !value.includes("\\") && !value.includes("\0");
}
/** The skill's root URI: its SKILL.md URI with the `/SKILL.md` suffix removed. */
function mcpSkillRoot(skillUri) {
	if (!skillUri.endsWith(SKILL_FILE_SUFFIX)) return void 0;
	const root = skillUri.slice(0, -9);
...
*/
function mcpRelativePath(skillUri, resourceUri) {
	if (resourceUri === skillUri) return "SKILL.md";
	const root = mcpSkillRoot(skillUri);
	if (root === void 0) return void 0;
	const prefix = `${root}/`;
	if (!resourceUri.startsWith(prefix)) return void 0;
	const relativePath = resourceUri.slice(prefix.length);
	const segments = relativePath.split("/");
	if (relativePath.length === 0 || relativePath.endsWith("/") || segments.some((segment) => segment.length === 0 || segment === "." || segment === ".." || /[<>:"|?*\0\\]/u.test(segment) || [...segment].some((character) => character.charCodeAt(0) < 32))) return;
	return relativePath;
}
function parseMcpSkillResource(value) {
	if (!isRecord(value)) return void 0;
	if (!isSafeUri(value.uri)) return void 0;
	if (typeof value.digest !== "string" || !MCP_DIGEST.test(value.digest)) return void 0;
	if (typeof value.size !== "number" || !Number.isSafeInteger(value.size) || value.size < 0) return void 0;
	return {
		uri: value.uri,
		digest: value.digest,
		size: value.size
...
*/
function validateMcpSkillEntry(value) {
	if (!isRecord(value)) return void 0;
	if (!isSafeUri(value.uri) || mcpSkillRoot(value.uri) === void 0) return void 0;
	const frontmatter = value.frontmatter;
	if (!isRecord(frontmatter)) return void 0;
	...
	if (mcpSkillRoot(value.uri).split("/").at(-1) !== name) return void 0;
	if (!Array.isArray(value.resources)) return void 0;
	if (value.resources.length === 0 || value.resources.length > 512) return void 0;
	const resources = [];
	const seen = /* @__PURE__ */ new Set();
	let totalBytes = 0;
	...
	for (const item of value.resources) {
		...
		if (resource === void 0 || seen.has(resource.uri)) return void 0;
		if (mcpRelativePath(value.uri, resource.uri) === void 0) return void 0;
		seen.add(resource.uri);
		if (resource.uri === value.uri) hasSkillFile = true;
		totalBytes += resource.size;
		if (totalBytes > 16777216) return void 0;
		resources.push(resource);
	}
	if (!hasSkillFile) return void 0;
	return {
		uri: value.uri,
		frontmatter: {
			...frontmatter,
...
function mcpContentBoundKey(entry) {
	return createHash("sha256").update([...entry.resources].sort((left, right) => left.uri.localeCompare(right.uri, "en")).map((resource) => JSON.stringify([
		resource.uri,
		resource.digest,
		resource.size
	])).join("\n")).digest("hex");
...
function mcpFrontmatterEqual(parsed, expected) {
...
function slugSegment(segment) {
	return segment.toLocaleLowerCase("en-US").replaceAll(/[^a-z0-9]+/gu, "-").replaceAll(/^-+|-+$/gu, "");
}
/**
...
*/
function resolvedMcpName(entry, taken) {
	const base = entry.frontmatter.name;
	const uriHash = createHash("sha256").update(entry.uri).digest("hex").slice(0, 6);
	const slug = (mcpSkillRoot(entry.uri) ?? "").split("/").slice(0, -1).map(slugSegment).filter((segment) => segment.length > 0).join("-");
	if (!taken.has(base)) {
		taken.add(base);
		return base;
	}
...
function mcpCandidates(serverLabel, entries, options = {}) {
	assertMcpServerLabel(serverLabel);
	const trusted = new Set((options.trustedServers ?? []).map((label) => label.toLocaleLowerCase("en-US")));
	const unique = /* @__PURE__ */ new Map();
	for (const entry of entries) if (!unique.has(entry.uri)) unique.set(entry.uri, entry);
	const ordered = [...unique.values()].sort((left, right) => left.frontmatter.name.localeCompare(right.frontmatter.name, "en") || left.uri.localeCompare(right.uri, "en"));
...
function completeResult(value, method) {
	if (!isRecord(value) || value.resultType !== "complete") throw new Error(`MCP ${method} returned an incomplete result`);
	return value;
}
var McpSkillsClient = class {
	transport;
	...
	*/
	async listSkills(signal) {
		...
		for (let page = 0; page < 10; page += 1) {
			signal?.throwIfAborted();
			const result = await this.transport.request("skills/list", cursor === void 0 ? {} : { cursor });
			signal?.throwIfAborted();
			const payload = completeResult(result, "skills/list");
			if (!Array.isArray(payload.skills)) throw new Error("MCP skills/list returned an invalid skills array");
	...
	async getSkill(uri, signal) {
		signal?.throwIfAborted();
		if (!isSafeUri(uri)) throw new Error(`invalid MCP skill uri "${uri}"`);
		const result = await this.transport.request("skills/get", { uri });
		signal?.throwIfAborted();
		const entry = validateMcpSkillEntry(completeResult(result, "skills/get").skill);
		if (entry === void 0 || entry.uri !== uri) throw new Error(`MCP skills/get did not return the requested skill "${uri}"`);
		return entry;
	}
	/**
	...
	*/
	async readResource(uri, signal) {
		...
		signal?.throwIfAborted();
		const payload = completeResult(result, "resources/read");
		if (!Array.isArray(payload.contents) || payload.contents.length === 0) throw new Error(`MCP resources/read returned no content for "${uri}"`);
		const contents = payload.contents.filter((item) => isRecord(item));
		const match = contents.find((item) => item.uri === uri) ?? (contents.length === 1 && contents[0]?.uri === void 0 ? contents[0] : void 0);
		if (match === void 0) throw new Error(`MCP resources/read returned no content matching "${uri}"`);
		...
		if (typeof match.blob === "string") {
			...
			if (encoded.length % 4 !== 0 || !/^(?:[A-Za-z0-9+/]{4})*(?:[A-Za-z0-9+/]{2}==|[A-Za-z0-9+/]{3}=)?$/u.test(encoded)) throw new Error(`MCP resources/read returned invalid base64 content for "${uri}"`);
...
function validManifest(value) {
	...
	if (origin !== void 0 && origin !== "github" && origin !== "mcp") return false;
	const mcpManifest = item.mcp;
	return item.version === 1 && typeof item.cacheId === "string" && CACHE_ID$1.test(item.cacheId) && typeof item.source === "string" && typeof item.ref === "string" && (origin === "mcp" ? /^[0-9a-f]{64}$/u.test(item.ref) : /^[0-9a-f]{40}$/u.test(item.ref)) && typeof item.skillId === "string" && typeof item.name === "string" && typeof item.description === "string" && (item.installs === void 0 || typeof item.installs === "number" && Number.isSafeInteger(item.installs) && item.installs >= 0) && (item.qualityScore === void 0 || typeof item.qualityScore === "number" && Number.isSafeInteger(item.qualityScore) && item.qualityScore >= 0 && item.qualityScore <= 100) && (item.trustLevel === void 0 || item.trustLevel === "unverified" || item.trustLevel === "community" || item.trustLevel === "corroborated" || item.trustLevel === "trusted") && (item.stars === void 0 || typeof item.stars === "number" && Number.isSafeInteger(item.stars) && item.stars >= 0) && (item.pushedAt === void 0 || typeof item.pushedAt === "string") && (item.discoverySources === void 0 || Array.isArray(item.discoverySources) && item.discoverySources.every((source) => source === "skills.sh" || source === "github")) && (item.sourcePath === void 0 || typeof item.sourcePath === "string" && item.sourcePath.length > 0) && (item.sourceSkillFileHash === void 0 || typeof item.sourceSkillFileHash === "string" && /^[0-9a-f]{64}$/u.test(item.sourceSkillFileHash)) && item.sourcePath === void 0 === (item.sourceSkillFileHash === void 0) && (origin === "mcp" ? typeof mcpManifest === "object" && mcpManifest !== null && (() => {
		const record = mcpManifest;
		return typeof record.serverLabel === "string" && typeof record.skillUri === "string" && typeof record.contentBoundKey === "string" && /^[0-9a-f]{64}$/u.test(record.contentBoundKey) && item.ref === record.contentBoundKey && typeof record.frontmatter === "object" && record.frontmatter !== null && Array.isArray(record.resources) && record.resources.every((resource) => {
			if (typeof resource !== "object" || resource === null) return false;
			const entry = resource;
			return typeof entry.uri === "string" && typeof entry.digest === "string" && /^sha256:[0-9a-f]{64}$/u.test(entry.digest) && typeof entry.size === "number" && Number.isSafeInteger(entry.size) && entry.size >= 0;
		});
	})() : mcpManifest === void 0) && typeof item.installedAt === "string" && Number.isFinite(Date.parse(item.installedAt)) && typeof item.fileCount === "number" && Number.isSafeInteger(item.fileCount) && item.fileCount >= 1 && typeof item.totalBytes === "number" && Number.isSafeInteger(item.totalBytes) && item.totalBytes >= 0 && typeof item.contentHash === "string" && /^[0-9a-f]{64}$/u.test(item.contentHash) && (item.whenToUse === void 0 || typeof item.whenToUse === "string");
}
function mcpCacheId(candidate) {
	return createHash("sha256").update(JSON.stringify([
		candidate.serverLabel,
		candidate.skillUri,
		candidate.contentBoundKey
	])).digest("hex").slice(0, 24);
}
async function verifyMcpManifestResources(entry, signal) {
	...
	for (const resource of mcp.resources) {
		...
		if (relativePath === void 0) throw new Error(`MCP cached skill contains a resource outside its directory: "${resource.uri}"`);
		const target = join(entry.directory, ...relativePath.split("/"));
		assertWithin(entry.directory, target);
		const bytes = await readFile(target);
		signal?.throwIfAborted();
		if (bytes.length !== resource.size) throw new Error(`MCP cached resource size changed for "${resource.uri}"`);
		if (`sha256:${createHash("sha256").update(bytes).digest("hex")}` !== resource.digest) throw new Error(`MCP cached resource digest mismatch for "${resource.uri}"`);
	}
}
...
var SkillCache = class {
	...
	async load(entry, signal) {
		if (entry.manifest.origin === "mcp") await verifyMcpManifestResources(entry, signal);
	...
	*/
	async installMcp(candidate, readResource, signal) {
		...
		try {
			if (candidate.resources.length === 0 || candidate.resources.length > this.options.maxFiles) throw new Error(`MCP skill exceeds the ${this.options.maxFiles}-file installation limit`);
			let totalBytes = 0;
			for (const resource of [...candidate.resources].sort((left, right) => left.uri.localeCompare(right.uri, "en"))) {
				operationSignal.throwIfAborted();
				const relativePath = mcpRelativePath(candidate.skillUri, resource.uri);
				if (relativePath === void 0) throw new Error(`MCP resource uri "${resource.uri}" is outside its skill directory`);
				const bytes = await withAbort(readResource(resource.uri, operationSignal), operationSignal);
				operationSignal.throwIfAborted();
				if (bytes.length !== resource.size) throw new Error(`MCP resource size changed for "${resource.uri}"`);
				if (`sha256:${createHash("sha256").update(bytes).digest("hex")}` !== resource.digest) throw new Error(`MCP resource digest mismatch for "${resource.uri}"`);
				totalBytes += bytes.length;
				if (totalBytes > this.options.maxBytes) throw new Error(`MCP skill exceeds the ${this.options.maxBytes}-byte installation limit`);
			...
			try {
				parsedFrontmatter = parseSkillFrontmatter(new TextDecoder("utf-8", { fatal: true }).decode(skillBytes));
			} catch {
				throw new Error("MCP skill SKILL.md has no parseable YAML frontmatter");
			}
			if (!mcpFrontmatterEqual(parsedFrontmatter, candidate.frontmatter)) throw new Error("MCP skill frontmatter does not match its skills/get entry");
			const inspected = await inspectSkillDirectory(downloaded, {
				maxFiles: this.options.maxFiles,
				maxBytes: this.options.maxBytes
			}, operationSignal);
			operationSignal.throwIfAborted();
			if (inspected.definition.name !== candidate.frontmatter.name) throw new Error(`MCP skill name "${inspected.definition.name}" does not match its entry`);
			const manifest = {
				version: 1,
				origin: "mcp",
				cacheId: id,
...
function catalogEntryLine(entry, maxLength) {
...
var DiscoveryCoordinator = class {
	...
	async withMcp(query, candidates, signal) {
		if (this.host.mcp === void 0 || this.host.config.mcpDiscovery === "off") return [...candidates];
	...
	async discover(agent, query, options = {}) {
		...
		options.signal?.throwIfAborted();
		if (options.remote !== true || this.host.config.remoteDiscovery === "off") return await this.withMcp(query, selected, options.signal);
		const remote = await this.host.remote.search(query, options.signal);
		options.signal?.throwIfAborted();
		return (await this.withMcp(query, dedupeById([...selected, ...remote]), options.signal)).slice(0, this.host.config.remoteSearchLimit * 2);
	}
...
const GITHUB_SOURCE$2 = /^[A-Za-z0-9_.-]+\/[A-Za-z0-9_.-]+$/u;
const SKILL_NAME$2 = /^[a-z0-9]+(?:-[a-z0-9]+)*$/u;
...
function isSearchItem(value) {
	if (typeof value !== "object" || value === null) return false;
	const item = value;
	return typeof item.skillId === "string" && SKILL_NAME$1.test(item.skillId) && typeof item.name === "string" && item.name.trim().length > 0 && item.name.length <= 4096 && typeof item.installs === "number" && Number.isSafeInteger(item.installs) && item.installs >= 0 && typeof item.source === "string" && GITHUB_SOURCE$1.test(item.source);
}
...
function isGithubCodeSearchItem(value) {
	...
	return typeof repository.full_name === "string" && GITHUB_SOURCE$1.test(repository.full_name) && repository.private === false;
}
...
var RemoteDiscoveryClient = class {
	...
	async searchLive(normalized, operationSignal) {
		...
		if (this.options.registryDiscovery === "automatic" && this.options.registry !== void 0 && this.providerAvailable("registry-index")) providerTasks.push(this.options.registry.listSeeds(operationSignal).then((listing) => {
		...
		const registrySeeds = fulfilled.flatMap((result) => result.provider === "registry-index" ? result.value.map((seed) => ({
			source: seed.source,
			skillId: seed.name,
			name: seed.name,
		...
		const candidates = seeds.flatMap((seed) => {
			...
			const evidence = remoteQualityEvidence({
				...
				...seed.registryTier === void 0 ? {} : { registryTier: seed.registryTier },
				now
			});
...
const REGISTRY_MAX_ENTRIES = 1e3;
const REGISTRY_MAX_DESCRIPTION_LENGTH = 4096;
function validateRegistryIndexEntry(value) {
	if (typeof value !== "object" || value === null) return void 0;
	const item = value;
	if (typeof item.name !== "string" || !SKILL_NAME.test(item.name)) return void 0;
	if (typeof item.description !== "string" || item.description.trim().length === 0 || item.description.length > 4096) return void 0;
	if (typeof item.source !== "string" || !GITHUB_SOURCE.test(item.source)) return void 0;
	if (typeof item.ref !== "string" || !GIT_REF.test(item.ref)) return void 0;
	if (typeof item.tier !== "string" || !TIERS.includes(item.tier)) return void 0;
	if (item.installs !== void 0 && (typeof item.installs !== "number" || !Number.isSafeInteger(item.installs) || item.installs < 0)) return void 0;
	if (item.license !== void 0 && typeof item.license !== "string") return void 0;
	if (item.whenToUse !== void 0 && (typeof item.whenToUse !== "string" || item.whenToUse.trim().length === 0)) return;
	return {
		name: item.name,
		...
		ref: item.ref,
		tier: item.tier,
		...item.installs === void 0 ? {} : { installs: item.installs },
...
*/
function resolveTokenMeter(ctx) {
	try {
		const meter = ctx.get("tokenMeter");
		if (meter === void 0 || typeof meter.estimateMessage !== "function") return void 0;
		return meter;
	} catch {
		return;
	}
...
function estimateWithMeter(meter, text) {
	if (text.length === 0) return {
		tokens: 0,
		estimator: meter === void 0 ? "portable" : "token-meter"
	};
	if (meter !== void 0) try {
...
*/
function estimateCatalogEntries(meter, skills, maxLength) {
	const estimates = skills.map((skill) => estimateWithMeter(meter, catalogEntryLine(skill, maxLength)));
	const tokens = estimates.reduce((total, estimate) => total + estimate.tokens, 0);
	const estimator = meter !== void 0 && estimates.every((estimate) => estimate.estimator === "token-meter") ? "token-meter" : "portable";
	return {
		tokens: Math.min(Number.MAX_SAFE_INTEGER, tokens),
		estimator
	};
}
//#endregion
...
function validRecord(value) {
	if (typeof value !== "object" || value === null) return false;
	const item = value;
	return boundedString(item.candidateId, 512) && boundedString(item.name, 128) && (item.origin === "registry" || item.origin === "cache" || item.origin === "remote" || item.origin === "mcp") && boundedString(item.source, 2048) && (item.cacheId === void 0 || typeof item.cacheId === "string" && CACHE_ID.test(item.cacheId)) && count(item.mounts) && count(item.uses) && timestamp(item.lastMountedAt) && timestamp(item.lastUsedAt) && (item.catalogFootprintTokens === void 0 || count(item.catalogFootprintTokens)) && (item.loadedBodyTokens === void 0 || count(item.loadedBodyTokens)) && (item.totalLoadedBodyTokens === void 0 || count(item.totalLoadedBodyTokens)) && timestamp(item.lastLoadedAt) && (item.tokenEstimator === void 0 || item.tokenEstimator === "token-meter" || item.tokenEstimator === "portable");
}
...
var UsageStore = class {
	...
	*/
	async recordTelemetry(identity, telemetry) {
		const loadedBodyTokens = telemetry.loadedBodyTokens;
		if (loadedBodyTokens !== void 0 && !count(loadedBodyTokens)) throw new Error("invalid loaded body token estimate");
		const catalogFootprintTokens = telemetry.catalogFootprintTokens;
		if (catalogFootprintTokens !== void 0 && !count(catalogFootprintTokens)) throw new Error("invalid catalog footprint token estimate");
		if (telemetry.estimator !== void 0 && telemetry.estimator !== "token-meter" && telemetry.estimator !== "portable") throw new Error("invalid token estimator kind");
		await this.enqueue(async (records) => {
			const previous = records.get(identity.candidateId) ?? identityRecord(identity);
			records.set(identity.candidateId, {
				...identityRecord(identity),
				...
				...previous.lastMountedAt === void 0 ? {} : { lastMountedAt: previous.lastMountedAt },
				...previous.lastUsedAt === void 0 ? {} : { lastUsedAt: previous.lastUsedAt },
				...previous.catalogFootprintTokens === void 0 ? {} : { catalogFootprintTokens: previous.catalogFootprintTokens },
				...previous.loadedBodyTokens === void 0 ? {} : { loadedBodyTokens: previous.loadedBodyTokens },
				...previous.totalLoadedBodyTokens === void 0 ? {} : { totalLoadedBodyTokens: previous.totalLoadedBodyTokens },
				...previous.lastLoadedAt === void 0 ? {} : { lastLoadedAt: previous.lastLoadedAt },
				...previous.tokenEstimator === void 0 ? {} : { tokenEstimator: previous.tokenEstimator },
				...loadedBodyTokens === void 0 ? {} : {
					loadedBodyTokens,
					totalLoadedBodyTokens: Math.min(Number.MAX_SAFE_INTEGER, (previous.totalLoadedBodyTokens ?? 0) + loadedBodyTokens),
					lastLoadedAt: this.currentTime()
				},
				...catalogFootprintTokens === void 0 ? {} : { catalogFootprintTokens },
				...telemetry.estimator === void 0 ? {} : { tokenEstimator: telemetry.estimator }
			});
		});
	}
	...
	async cacheEvidence() {
		...
		return [...(await this.readLatest()).values()].map((record) => ({
			...
			...record.totalLoadedBodyTokens === void 0 ? {} : { totalLoadedBodyTokens: record.totalLoadedBodyTokens }
		}));
	}
...
var McpStdioTransport = class {
	...
	request(method, params) {
		if (this.exited) return Promise.reject(new McpError(-32e3, "MCP server has exited"));
		this.ensureChild();
		...
		return new Promise((resolve, reject) => {
			const timer = setTimeout(() => {
				...
				reject(new McpError(-32e3, `MCP request ${method} timed out`));
			}, this.timeoutMs);
			this.pending.set(id, {
				resolve,
				reject,
	...
	ensureChild() {
		...
		child.once("error", (error) => {
			this.exited = true;
			this.rejectAll(new McpError(-32e3, `MCP server spawn failed: ${error.message}`));
		});
		child.once("exit", (code) => {
			this.exited = true;
			this.rejectAll(new McpError(-32e3, `MCP server exited with code ${code ?? "unknown"}`));
		});
	...
	settle(raw) {
		...
		if (message.error !== void 0) {
			const code = typeof message.error.code === "number" ? message.error.code : -32e3;
...
function mcpServerLabels(name, values) {
	const labels = values.map((value) => value.trim()).filter((value) => value.length > 0);
	for (const label of labels) if (!/^[A-Za-z0-9][A-Za-z0-9._-]{0,63}$/u.test(label)) throw new Error(`dsh-skillflux: invalid MCP server label "${label}" in ${name}`);
	return [...new Set(labels.map((label) => label.toLocaleLowerCase("en-US")))].sort((left, right) => left.localeCompare(right, "en"));
}
...
function resolveConfig(config) {
	...
	if (conflictingServer !== void 0) throw new Error(`dsh-skillflux: MCP server "${conflictingServer}" cannot be both trusted and blocked`);
	return resolved;
}
...
var SkillFluxService = class extends Service {
	...
	static Config = z.object({
		...
		remoteProviders: z.array(z.union([
			"skills.sh",
			"github",
			"registry-index"
		])).default([...DEFAULTS.remoteProviders]),
		...
		mcpDiscovery: z.union(["automatic", "off"]).default(DEFAULTS.mcpDiscovery),
		mcpTrustedServers: z.array(z.string()).default([]),
		mcpBlockedServers: z.array(z.string()).default([]),
		registryDiscovery: z.union(["automatic", "off"]).default(DEFAULTS.registryDiscovery),
	...
	discovery;
	providers;
	mcpSources = /* @__PURE__ */ new Map();
	registryIndexes = /* @__PURE__ */ new Map();
	...
	constructor(ctx, config = {}) {
		...
		const registryPort = { listSeeds: async (signal) => {
			...
			for (const label of labels) try {
				...
				signal?.throwIfAborted();
				this.runtimeCtx.logger.warn(`SkillFlux registry index "${label}" discovery failed open: ${errorMessage(error)}`);
				partial = true;
			}
			const seen = /* @__PURE__ */ new Set();
			return {
	...
	get discoveryHost() {
		return {
			...
			usage: this.usage,
			mcp: { listCandidates: (query, signal) => this.listMcpCandidates(query, signal) }
		};
	}
	get activationHost() {
		return {
			...
			recordMountTelemetry: (state, mounted) => {
				this.recordMountTelemetry(state, mounted);
			},
			scheduleAutoPrune: () => {
	...
	recordMountTelemetry(state, mounted) {
		if (this.usage === void 0) return;
		const meter = resolveTokenMeter(this.runtimeCtx);
		const body = estimateWithMeter(meter, mounted.definition.content);
		const catalog = estimateCatalogEntries(meter, this.catalogSkills(state), this.config.catalogDescriptionMaxLength);
	...
	*/
	registerRegistryIndex(label, transport) {
		if (!/^[A-Za-z0-9][A-Za-z0-9._-]{0,63}$/u.test(label)) throw new Error(`invalid host-assigned registry index label "${label}"`);
		this.registryIndexes.set(label, new RegistryIndexClient(transport));
		this.providers.invalidate();
	}
	unregisterRegistryIndex(label) {
	...
	async executeCommand(invocation) {
		...
		if (parts.length === 1 && parts[0] === "status") {
			...
			const tokenTelemetry = `Token telemetry: ${resolveTokenMeter(this.runtimeCtx) === void 0 ? "portable fallback" : "native token-meter"}.`;
		...
		if (parts.length === 1 && parts[0] === "usage") {
			...
			return {
				kind: "success",
				text: !this.config.usageTracking ? "SkillFlux usage tracking is disabled." : records.length === 0 ? "SkillFlux has no usage statistics yet." : `SkillFlux usage (top ${records.length}):\n${records.map((record) => `- ${record.name} (${record.origin}, ${record.source}): uses ${record.uses}, mounts ${record.mounts}, last used ${formatTimestamp(record.lastUsedAt)}${record.totalLoadedBodyTokens === void 0 ? "" : `, body tokens ${record.totalLoadedBodyTokens} (${record.tokenEstimator ?? "portable"})`}`).join("\n")}`
			};
		}
	...
	async routeTurn(agent, task, turn, explicit, signal) {
		...
		if (this.config.remoteDiscovery !== "automatic") {
			...
			const publishableMcp = mcp.candidates.filter((candidate) => candidateGovernanceReason(candidate, this.config) === void 0).slice(0, Math.min(this.config.remoteAutoMountLimit, mcpSlots));
			state.published = {
				candidates: [...published, ...publishableMcp],
		...
		const publishableMcp = mcp.candidates.filter((candidate) => candidateGovernanceReason(candidate, this.config) === void 0).slice(0, Math.min(this.config.remoteAutoMountLimit, Math.max(0, remainingSlots - publishable.length)));
		state.published = {
			candidates: [
				...published,
				...publishable,
	...
	publishedCandidate(agent, name) {
		const candidate = this.turnStates.peek(agent)?.published.candidates.find((item) => item.name === name);
		if (candidate === void 0 || candidate.origin !== "remote" && candidate.origin !== "mcp") return void 0;
		return candidate;
	}
	async readMcpResource(label, uri, signal) {
		const client = this.mcpSources.get(label);
		if (client === void 0) throw new Error(`MCP source "${label}" is not registered`);
	...
	*/
	async listMcpCandidates(query, signal) {
		if (this.config.mcpDiscovery === "off" || this.mcpSources.size === 0) return {
			candidates: [],
			complete: true
		};
		...
		const outcomes = await Promise.all(labels.map(async (label) => {
			...
			try {
				...
				signal?.throwIfAborted();
				this.runtimeCtx.logger.warn(`SkillFlux MCP source "${label}" discovery failed open: ${errorMessage(error)}`);
				return {
					label,
					status: "error"
				};
		...
		const candidates = outcomes.flatMap((outcome) => {
			...
			return mcpCandidates(outcome.label, outcome.entries, { trustedServers: this.config.mcpTrustedServers }).flatMap((candidate) => {
				const baseScore = routeScore(query, candidate);
				...
				if (candidateGovernanceReason(scored, this.config) !== void 0) return [];
				return [scored];
			});
		});
		candidates.sort((left, right) => right.score - left.score || left.name.localeCompare(right.name, "en") || left.serverLabel.localeCompare(right.serverLabel, "en"));
		return {
			candidates,
			complete
		};
	...
	*/
	async collectMcpCandidates(state, task, turn, signal) {
		let found = [];
		let complete = true;
		try {
		...
		const accepted = [];
		for (const candidate of found) if (this.catalogFitsBudget(state, candidate)) accepted.push(candidate);
		else state.lastRouting.push({
			...routingTrace(candidate, turn),
			outcome: "budget-skipped"
		});
	...
	async loadOne(agent, candidate, signal) {
		...
		try {
			...
			if (candidate.origin === "cache") {
				...
				entry = await this.cache.installMcp(candidate, (uri, readSignal) => this.readMcpResource(candidate.serverLabel, uri, readSignal), cacheSignal);
				cacheSignal?.throwIfAborted();
			} else {
			...
			if (this.usage !== void 0) {
				...
				const catalog = estimateCatalogEntries(meter, stateForCatalog === void 0 ? [{
					name: definition.name,
					description: definition.description
				}] : this.catalogSkills(stateForCatalog), this.config.catalogDescriptionMaxLength);
				this.trackUsage(this.usage.recordTelemetry({
					candidateId: candidate.id,
					name: definition.name,
					origin: candidate.origin,
					source: candidate.source,
					...entry.manifest.cacheId === void 0 ? {} : { cacheId: entry.manifest.cacheId }
				}, {
					loadedBodyTokens: body.tokens,
					catalogFootprintTokens: catalog.tokens,
					estimator: body.estimator
