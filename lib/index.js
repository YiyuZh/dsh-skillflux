...
function isSearchItem(value) {
	if (typeof value !== "object" || value === null) return false;
	const item = value;
	return typeof item.skillId === "string" && SKILL_NAME.test(item.skillId) && typeof item.name === "string" && item.name.trim().length > 0 && typeof item.installs === "number" && Number.isSafeInteger(item.installs) && item.installs >= 0 && typeof item.source === "string" && GITHUB_SOURCE.test(item.source);
}
...
var RemoteDiscoveryClient = class {
	...
	async search(query, signal) {
		...
		const snapshots = await resolveRepositories([.../* @__PURE__ */ new Set([...skillsSeeds.map((seed) => seed.source), ...githubHits.map((hit) => hit.repository.full_name)])], operationSignal, this.githubToken);
		operationSignal.throwIfAborted();
...
var SkillFluxService = class extends Service {
	...
	async executeCommand(invocation) {
		...
		return {
			kind: "error",
			text: "Usage: /skillflux status | explain | usage | cache list | cache clean <cache-id|all>"
		};
	}
...
//#endregion
export { EmbeddingRouter, RemoteDiscoveryClient, SkillCache, SkillFluxService, SkillFluxService as default, UsageStore, estimateCatalogTokens, estimateTextTokens, inspectSkillDirectory, isLoopbackProxyFailure, name, normalizeText, parseSkillMarkdown, remoteQualityScore, routeScore, selectCandidates, tokenize };
