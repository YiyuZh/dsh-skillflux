...
var SkillFluxService = class extends Service {
	...
	async routeTurn(agent, task, turn, explicit, signal) {
		...
		const localPool = [...registryCandidates(snapshot.skills.filter(isModelInvocable)), ...cacheCandidates(await this.cache.list())].filter((candidate) => !explicit.has(candidate.name));
	...
	async mountCandidate(state, candidate, signal) {
		...
		if (existingSummary !== void 0 && isModelInvocable(existingSummary)) {
			const definition = await this.runtimeCtx.skills.get(candidate.name, lookup);
