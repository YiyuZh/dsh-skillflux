...
function validManifest(value) {
	...
	const item = value;
	return item.version === 1 && typeof item.cacheId === "string" && CACHE_ID$1.test(item.cacheId) && typeof item.source === "string" && typeof item.ref === "string" && /^[0-9a-f]{40}$/u.test(item.ref) && typeof item.skillId === "string" && typeof item.name === "string" && typeof item.description === "string" && (item.installs === void 0 || typeof item.installs === "number" && Number.isSafeInteger(item.installs) && item.installs >= 0) && (item.qualityScore === void 0 || typeof item.qualityScore === "number" && Number.isSafeInteger(item.qualityScore) && item.qualityScore >= 0 && item.qualityScore <= 100) && (item.stars === void 0 || typeof item.stars === "number" && Number.isSafeInteger(item.stars) && item.stars >= 0) && (item.pushedAt === void 0 || typeof item.pushedAt === "string") && (item.discoverySources === void 0 || Array.isArray(item.discoverySources) && item.discoverySources.every((source) => source === "skills.sh" || source === "github")) && typeof item.installedAt === "string" && Number.isFinite(Date.parse(item.installedAt)) && typeof item.fileCount === "number" && Number.isSafeInteger(item.fileCount) && item.fileCount >= 1 && typeof item.totalBytes === "number" && Number.isSafeInteger(item.totalBytes) && item.totalBytes >= 0 && typeof item.contentHash === "string" && /^[0-9a-f]{64}$/u.test(item.contentHash) && (item.whenToUse === void 0 || typeof item.whenToUse === "string");
}
...
var SkillFluxService = class extends Service {
	...
	async executeCommand(invocation) {
		...
		if (parts.length === 1 && parts[0] === "status") {
			...
			const discovery = `Remote discovery: ${this.config.remoteDiscovery}; providers ${this.config.remoteProviders.map((provider) => provider === "github" && !this.remote.githubSearchEnabled ? "github (token unavailable)" : provider).join(", ")}; quality >= ${this.config.remoteMinQualityScore}; stars >= ${this.config.remoteMinStars}; recent window ${this.config.remoteRecentActivityDays} days.`;
