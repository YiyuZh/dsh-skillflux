...
async function inspectSkillDirectory(directory, limits) {
	let fileCount = 0;
	let totalBytes = 0;
	...
	return {
		definition: parseSkillMarkdown(await readFile(skillPath, "utf8"), directory),
		fileCount,
		totalBytes,
...
function validManifest(value) {
	...
	const item = value;
	return item.version === 1 && typeof item.cacheId === "string" && CACHE_ID.test(item.cacheId) && typeof item.source === "string" && typeof item.ref === "string" && /^[0-9a-f]{40}$/u.test(item.ref) && typeof item.skillId === "string" && typeof item.name === "string" && typeof item.description === "string" && typeof item.installedAt === "string" && typeof item.fileCount === "number" && Number.isSafeInteger(item.fileCount) && item.fileCount >= 1 && typeof item.totalBytes === "number" && Number.isSafeInteger(item.totalBytes) && item.totalBytes >= 0 && typeof item.contentHash === "string" && /^[0-9a-f]{64}$/u.test(item.contentHash) && (item.whenToUse === void 0 || typeof item.whenToUse === "string");
}
...
var SkillCache = class {
	...
	async clean(selector, active = /* @__PURE__ */ new Set()) {
		const entries = selector === "all" ? await this.list() : [await this.get(selector)].filter((entry) => entry !== void 0);
...
function remoteCandidateMessage(agent, candidates) {
	const published = agent.session.events.some((event) => event.type === "user/message" && event.data.source.kind === "skillflux-candidates");
	...
	return createUserMessage({
		content: [{
			...
			text: [
				"<system-reminder>",
				published ? "This remote SkillFlux candidate list replaces every earlier candidate list:" : "No installed skill matched. SkillFlux found these immutable remote candidates:",
				"",
...
function isSearchItem(value) {
	if (typeof value !== "object" || value === null) return false;
	const item = value;
	return typeof item.skillId === "string" && /^[a-z0-9]+(?:-[a-z0-9]+)*$/u.test(item.skillId) && typeof item.name === "string" && typeof item.installs === "number" && Number.isSafeInteger(item.installs) && item.installs >= 0 && typeof item.source === "string" && /^[A-Za-z0-9_.-]+\/[A-Za-z0-9_.-]+$/u.test(item.source);
}
...
var SkillFluxService = class extends Service {
	...
	constructor(ctx, config = {}) {
		...
		ctx.on("agent/pre-step", async ({ agent, messages, turn, step, signal }, next) => {
			...
			if (task === void 0) return decision;
			try {
				const hint = await this.routeTurn(agent, task, turn, signal);
	...
	async discover(agent, query, options = {}) {
		const installed = (await this.runtimeCtx.skills.snapshot(skillLookup(agent, options.signal))).skills.filter(isModelInvocable);
	...
	async reload(agent, name, signal) {
		...
		const candidate = (await this.discover(agent, name, signal === void 0 ? {} : { signal })).find((item) => item.name === name);
	...
	async routeTurn(agent, task, turn, signal) {
		const state = this.state(agent);
		if (state.turn !== turn) {
			this.cleanupState(state, false);
			state.turn = turn;
		...
		if (!snapshot.complete) return void 0;
		const selected = selectCandidates(task, dedupeByName([...registryCandidates(snapshot.skills.filter(isModelInvocable)), ...cacheCandidates(await this.cache.list())]), {
			limit: this.config.maxActiveSkills,
			minScore: this.config.minRouteScore,
			routes: this.config.routes
		});
		for (const candidate of selected) {
			state.candidates.set(candidate.id, candidate);
			await this.mountCandidate(state, candidate, signal);
		}
		if (state.active.size > 0 || this.config.remoteDiscovery !== "automatic") return void 0;
		let remote;
		try {
		...
		if (remote.length === 0) return void 0;
		...
		return remoteCandidateMessage(agent, remote);
	}
	async mountCandidate(state, candidate, signal) {
		...
		if (existingSummary !== void 0) {
			const definition = await this.runtimeCtx.skills.get(candidate.name, lookup);
			if (definition === void 0) throw new Error(`skill "${candidate.name}" disappeared during mount`);
			const mounted = {
				name: candidate.name,
				origin: "registry",
		...
		if (candidate.origin === "cache") {
			...
			entry = cached;
		} else entry = await this.cache.install(candidate, signal);
	...
	cleanupSession(session) {
		for (const state of this.states) if (state.agent.session === session) this.cleanupState(state, false);
	}
