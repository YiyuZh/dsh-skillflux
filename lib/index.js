...
function registerApprovalGate(ctx, host) {
	ctx.on("tools/pre-execute", async (exec, next) => {
		if (exec.name !== "skillflux_mount") return await next();
...
var TurnStateRegistry = class {
	...
	cleanupState(state, forget) {
		...
		for (const dispose of [...state.disposers.values()].reverse()) try {
			...
			this.warn(`SkillFlux unmount failed: ${errorMessage$5(error)}`);
		}
		...
		try {
			...
			if (state.active.get(mounted.name) === mounted) {
				...
				try {
					...
					host.runtimeCtx.logger.warn(`SkillFlux cancelled mount rollback failed: ${errorMessage$4(disposeError)}`);
				}
...
var DiscoveryCoordinator = class {
	...
	async discover(agent, query, options = {}) {
		const snapshot = await this.host.runtimeCtx.skills.snapshot(skillLookup(agent, options.signal));
	...
	async selectLocalCandidates(query, candidates, limit, semanticTrigger, signal) {
		...
		if (this.host.config.adaptiveRouting && this.host.usage !== void 0) try {
			...
			signal?.throwIfAborted();
			this.host.runtimeCtx.logger.warn(`SkillFlux adaptive routing failed open: ${errorMessage$3(error)}`);
		}
		...
		try {
			...
			signal?.throwIfAborted();
			this.host.runtimeCtx.logger.warn(`SkillFlux embedding routing failed open: ${errorMessage$3(error)}`);
			return lexical;
		}
	}
};
