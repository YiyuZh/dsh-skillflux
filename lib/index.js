...
var SkillFluxProvider = class {
	...
	async get(candidate, options) {
		...
		if (chain === void 0) throw new Error("SkillFlux candidate locator is unknown or expired");
...
var SkillFluxProviderManager = class {
	...
	*/
	async load(agent, name, signal) {
		if (!this.handles.has(agent)) return void 0;
...
var SkillFluxService = class extends Service {
	...
	constructor(ctx, config = {}) {
		...
		this.providers = new SkillFluxProviderManager((agent) => ({
			catalog: () => this.turnStates.peek(agent)?.published ?? {
				candidates: [],
				complete: true
			},
		...
		ctx.on("agent/pre-step", async ({ agent, signal }, next) => {
			...
			const skills = [...active, ...published].slice(0, this.config.maxActiveSkills);
			return {
				kind: "enter",
	...
	async routeTurn(agent, task, turn, explicit, signal) {
		...
		if (state.active.size > 0 || published.length > 0 || this.config.remoteDiscovery !== "automatic") return updateRemoteCandidates(agent, []);
		...
		state.published = {
			candidates: remote.filter((candidate) => candidateGovernanceReason(candidate, this.config) === void 0).slice(0, this.config.remoteAutoMountLimit),
			complete: true
		};
		this.providers.invalidate(agent);
